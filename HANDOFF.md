...
- SUB LIVE 底下至少一個主辦方用英文地址，城市判斷邏輯只認中文城市名稱，這場 `city` 會落成「未知」（2026-09-21 觀察到的單一案例，之後沒再看過同類情況，還沒決定要不要為英文地址另外寫規則）。

**需要持續維護、不是一次性任務**：
- `NOISE_KEYWORDS`（非音樂雜訊過濾）跟 `OUTSIDE_TAIWAN_VENUE_RE`（海外場次地理過濾）都是「看到新的就加」的清單，KKTIX 掃描範圍每次擴大都可能冒出新的漏網案例，這是預期中的常態工作，不是缺陷。
