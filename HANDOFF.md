...
寫於 2026-09-15，最近一次大整理 2026-09-25。**這份文件的目的**：讓一個完全沒看過對話紀錄的人（未來的你、另一台電腦的全新 session）在 5 分鐘內知道現在做到哪、能不能信任目前的程式碼、下一步該做什麼。

**怎麼讀這份文件**：只有下面「現在的狀態」跟「目前已知還沒解決的問題」這兩節是隨時反映最新狀況、需要每次都讀的。再往下（從「你打開這個 repo 應該先做的事」開始）是照時間順序寫的完整開發記錄——保留是因為裡面有大量真實查證過的根因分析、踩過的坑，之後遇到類似狀況很有參考價值，但**內文本身不會回頭更新**，讀到「還沒做」「尚未解決」字樣不代表現在還是這樣，一律以上面兩節為準。歷史記錄裡的章節標題大致照日期排列，不需要整份從頭讀到尾，用關鍵字搜尋通常比較快。
...
- **9/26（第五筆）**：修好 KKTIX group-hub 頁面重複卡片的 bug（真實案例：花澤香菜台北公演明明已開賣，畫面上卻多一張卡卡在「尚未開賣」）——KKTIX 有些頁面自己不賣票、只連到真正賣票的子頁面，這種頁面以前會被當成獨立卡片收進資料，若它的子頁面又剛好被獨立抓到，同一場演出就會出現重複卡，而且這張多餘的卡因為沒有自己的票，狀態會永遠卡住不會被排程複查更新。新增 `findChildEventIds()`／`isRedundantGroupHub()`（`scripts/adapters/kktix.mjs`）：只有當一個頁面完全沒有自己的票、而且它連到的每個子頁面這次都有被獨立抓到時，才在抓取收尾時把它排除；只要有子頁面沒被獨立抓到就保留（避免刪掉唯一的入口）。測試涵蓋（`kktix.test.mjs` 新增 6 個測試，用真實頁面 HTML 結構驗證，`npm test` 336 個全過），另外用一次性腳本對真實頁面驗證過（跑完即刪）。手動移除了這個 bug 唯一已知的現存重複卡（`data/events.json`，id `ca414f99a3edbd63`），之後同類情況會在抓取時自動擋掉。

- **9/26（第六筆）**：篩選器改版（Max 需求：地區合併＋多選＋點擊已選項目取消選取）。①城市篩選的「台北」「新北」合併成一個「雙北」選項——`src/filter.js` 新增 `cityBucket()`／`DOUBLE_BEI_CITIES`，只影響篩選比對，`event.city` 本身還是原本抓到的「台北」/「新北」，卡片顯示不受影響。②五個篩選器（城市/月份/類型/音樂人地區/價格）全部改成可以多選，同一篩選器內多個選項是「或」的關係（例如城市選「雙北」＋「高雄」＝顯示這兩個地區），不同篩選器之間仍是「且」的關係，跟原本一樣。③`src/interactions.js` 的 `openFilterSheet()` 從「單選、點了就關閉」改成「多選、點擊已選項目會取消選取，點擊再次套用，視窗維持開啟直到按『完成』」——只有第一個「全部XX」選項例外，點它會清空所有選取並立刻關閉（原本的「重設」行為維持不變）。`viewFilters`（`liveradar:view_filters` localStorage）的每個欄位從單一值改成陣列；`filter.js` 的 `passesViewFilters` 同時接受單一值和陣列（`toValues()` 正規化），舊的已存 localStorage 資料或用單一值呼叫的測試都不用遷移就能繼續動。測試涵蓋（`src/filter.test.js` 新增 8 個測試，涵蓋雙北合併比對、多選 OR 邏輯、清空篩選，`npm test` 342 個全過），並在本機 dev server 用真實資料手動驗證過：城市清單正確只顯示「雙北」不再分開列台北/新北、多選城市（雙北＋高雄）正確疊加、點擊已選項目正確取消、按「全部城市」正確清空並關閉視窗（驗證完已清掉測試用的 `liveradar:view_filters`，沒有動到 `data/events.json`）。

- **9/26（第七筆）**：修好「2026 囪擊音樂祭」重複卡片（跟花澤香菜那筆一模一樣的 KKTIX group-hub bug，只是這次卡在 `city: 未知` 而不是「尚未開賣」——都是舊資料在解析邏輯變好「之前」抓到的，之後排程只複查狀態不會回頭修）；已用 `isRedundantGroupHub()` 驗證過真的會被擋下，手動清掉這筆已卡住的舊資料。新增 **`npm run health-check`**（`scripts/health-check.mjs`）：Max 問「要怎麼防範這種問題」後決定寫的——掃 `data/events.json` 揪出城市未知／場館空白／售票中卻缺票價／沒辨識出主唱／疑似重複（同日期＋時間＋場館但 id 不同，就是這兩次真實 bug 的形狀）5 種可疑資料，唯讀不會自動改，列成清單給人工判斷。測試涵蓋（`scripts/health-check.test.mjs` 8 個測試，`npm test` 351 個全過）。**第一次真的對 `data/events.json` 跑了一次**：找到 3 筆城市未知、4 筆場館空白、18 筆缺票價、5 組疑似重複（其中至少幾組——林鼓子、小宮有紗、韓東、瑋瑋、OrmFolk——看起來很像同一種 KKTIX group-hub 重複卡，還沒逐筆查證處理，留給下次）。建議定期（比如每週）手動跑一次，或以後考慮接進 `liveradar-daily-fetch` 排程自動跑完貼結果，這次沒做。
...
- `pushToSupabase()` 無條件覆蓋雲端資料，同時開兩個分頁編輯可能互相覆蓋——只有 Max 一人使用、單一裝置，實務影響極低，完全解決要上 CRDT 等級的機制，對個人專案不成比例。
- KKTIX 完全沒被主辦方標任何分類的活動（例如 iri），目前仍只能靠 `ORG_PAGE_VENUES` 白名單找到；9/25 新增的「其他」分類掃描只能抓到「完全沒有標籤」的，如果活動被標了某個跟音樂無關的既有分類（例如誤標成「學習」），一樣抓不到。

**小、低優先度、觀察到但沒決定要不要處理**：
- `data/artists.yml` 裡約 64 筆藝人分類已經補上查證依據，但目前沒有任何場次在用（純資料庫存貨）；有新場次用到時，正常查證流程會自然覆蓋到，不用主動處理。
- SUB LIVE 底下至少一個主辦方用英文地址，城市判斷邏輯只認中文城市名稱，這場 `city` 會落成「未知」（2026-09-21 觀察到的單一案例，這次（9/25）查 city:未知 清單時沒再看到這個案例，可能已經自然解決，也可能只是巧合沒抓到，不確定）。

**2026-09-25 city:未知 清單追查結果（原本 12 筆降到 5 筆，剩下的都是真的該顯示未知）**：
- ~~KKTIX 場地欄位只有地址、沒有場地名稱時，城市判斷會完全失效~~ ✅ 已修好。`parseKktixVenue` 沒有 "/" 分隔符時整串地址會被誤判成場地名稱，`cityFromAddress` 因此收到空字串（真實案例：TECHNO BUS，地址明明寫著「台中市」）。改成 addressPart 找不到城市時，再把 venuePart 本身當地址試一次。
- ~~X-Formosa 彩虹音樂節的場地欄位抓到網頁分頁標籤文字~~ ✅ 已修好。iNDIEVOX 的 `parseVenueLine` 標籤比對太寬鬆，頁面自己的分頁導覽列「場地交通住宿｜活動時程｜表演者｜注意事項」裡「場地」後面接的「交通住宿」被當成標籤填充字，後面整串分頁名稱被誤判成場地內容。排除這一個已確認的假陽性複合詞（不是通用規則——類似的「票價資訊 Ticket Price：」這種合法標籤後面也會接更多中文字，沒辦法用「後面不能接中文字」這種通用規則解決，只能排除已知案例）。套用新邏輯後這個頁面真的沒有可解析的場地資訊，改用標題比對 `venues.yml`（原本 9/21 就有「X-Formosa」這筆，但因為 venue_raw 非空所以從沒機會觸發），現在正確顯示新北。
- ~~漁光島（TINY ISLAND 小島沙灘派對）、金滿山露營山莊（奇蹟 MIRACLE FESTIVAL）沒收錄進 venues.yml~~ ✅ WebSearch 查證後補上（台南、苗栗）。
- 剩下 5 筆城市未知都是真的該顯示未知，沒有勉強塞資料：兩場邱冠瑜線上音樂會（線上活動，沒有實體地點）、蕭景鴻多場地巡演合併售票頁、囪擊音樂祭的總覽頁（已正確拆出的 3 個確切日期場次都有 city:台南，這筆是額外的總覽/父頁面）、MOB PARTY 26（辦在泰國清邁，OUTSIDE_TAIWAN_VENUE_RE 目前沒有涵蓋泰國，但只有這一筆觀察案例，還沒決定要不要加）。

**需要持續維護、不是一次性任務**：
- `NOISE_KEYWORDS`（非音樂雜訊過濾）跟 `OUTSIDE_TAIWAN_VENUE_RE`（海外場次地理過濾）都是「看到新的就加」的清單，KKTIX 掃描範圍每次擴大都可能冒出新的漏網案例，這是預期中的常態工作，不是缺陷。**2026-09-25 又一次證明這句話**：查 city:未知 清單時發現麥花臣場館、亞洲國際博覽館（AsiaWorld-Expo）兩個香港場地從來沒被真的加進規則裡（只在程式碼註解裡提過），兩場香港秀因此真的顯示在正式網站上，不是卡在待整理；已經補上規則，並清掉包含一場規則早就涵蓋、但因為是舊資料從沒被重新檢查過的 AXA WONDERLAND 香港場（同一種「增量抓取不會回頭檢查舊資料」的坑，這個 session 已經是第 N 次踩到），共移除 3 場。
...
## 里程碑總覽（歷史記錄——現況以上面「現在的狀態」為準）

**M9 已經過時**：Gist 同步後來整個被 Supabase 帳號登入取代（見下方「Supabase 帳號登入上線」），下面這列描述的機制已經不存在，保留只是留紀錄。
...
| M9 | ~~Gist 同步~~（連接/斷開/雙向同步/離線 fallback）＋ FR-63/64 匯出匯入、設定頁的嚴格模式與靜音關鍵字順便一起接上 | 🗑️ 已被 Supabase 帳號登入取代，見「Supabase 帳號登入上線」章節 |
| M10 | 來源異常告警（GitHub issue）、設定頁來源狀態儀表、時間表異常 banner | ✅（真的開 issue 那段沒有跑過真實 CI，見下方說明） |
| M11 | GitHub Actions 排程上線 | ✅ 當時排程已重新打開；後來（9/21）確認拓元/Ticket Plus 會被 GitHub Actions 的 IP 擋掉，排程改到上班電腦的 Claude 排程任務，GitHub Actions 排程本身已經不再使用 |
...
`npm run fetch` 會打五個平台的真實網站。2026-09-24 優化前一輪約 30 分鐘，優化後預估 10~15 分鐘（實測數字見文件尾端 9/24 章節）。**一天只跑一次**，改了抓取程式要驗證用 `npm run fetch-one`，補完藝人用 `npm run renormalize`，跑完覆寫 `data/*.json`。`data/artists.yml` 已經有 400 多筆真實藝人；辨識不到藝人的場次一樣會上架（D15 reversal），只是會列進 `data/needs-review.json` 等待查證補登記。（2026-09-23 更正：這裡原本寫「`artists.yml` 只有兩筆示範資料，fetch 完 `events.json` 一定是空的」，那是專案最早期的狀況，早就不成立了。）
...
10. **同一時間只能有一個抓取在跑**（2026-09-24 起有鎖定檔保護）：`npm run fetch` 跟 `npm run renormalize` 開始時會建 `.fetch.lock`，另一個啟動時如果發現有人在跑，會印出是誰、跑多久了，然後以結束碼 2 停止，不會兩邊同時改 `data/*.json`。9/23 兩個 session 加排程在同一個資料夾同時跑，差點把改動弄丟，也讓請求量翻倍。前一個程序被強制中止留下的鎖定檔會自動接手，不用手動刪。**注意這只保護抓取，不保護程式碼**：兩個 session 同時改同一個 git 工作目錄還是會互相覆蓋，開新 session 前先確認另一個已經收尾（`git status` 乾淨）。
11. **查證藝人身分要防撞名，不能用名字判斷國籍**（2026-09-24）：搜尋結果要跟票券頁對照（主辦、類型、巡演名、頁面自己寫的出身、同場演出者），至少一項對得上才能寫進 artists.yml，註解寫出對照了什麼、在哪查到。過去查錯的四種模式：從名字猜（W!nky；琉璃花 9/23 因「RURIKA 是日文讀音」被標成日本，其實是台灣日系地下偶像）、把演出地點當國籍（JUST YOU & WIN 辦在大阪但是泰國演員）、把巡演企劃名當藝人（Yoshinani）、撞名（搜到同名的另一個人）。排程指令已同步寫入這條規則。
12. **來源地看「在哪個音樂圈成名、被認識的」**（2026-09-24 Max 定的規則）：不是國籍，也不是現在住哪或演出地；成名後才移居的仍算原本成名的地方；查不出在哪成名才退回用國籍。案例：蔡健雅（新加坡籍）、巫啟賢（馬來西亞籍）在台灣成名 → 本地；韓東（中國籍，Dreamcatcher 成員）在 K-pop 圈成名 → 韓國；ONE OK ROCK 後來長年在歐美發展，仍是日本。排程指令已同步。
13. **派對、音樂節一律判定本地**（2026-09-24 Max 定的規則）：多位音樂人參與，無法用單一藝人判斷，直接登記品牌名為本地（例：SLUTs 派對）。排程指令已同步。
14. **非音樂人的見面會、夜店派對都要收錄**（2026-09-24 Max 決定）：多少跟音樂或偶像有關，不要加進 NOISE_KEYWORDS 排除。見面會照常查主角在哪成名；夜店派對登記品牌名為本地。排程指令已同步。
...
🗑️ **這整節描述的 Gist token 同步機制已經在 2026-09-20 被 Supabase 帳號登入整個取代**（`reconcileGistSync()`／`gist_id` 等都已經從程式碼裡刪掉），下面內容純粹是歷史記錄，不要照著去測或去修——現在要驗證跨裝置同步，看「Supabase 帳號登入上線」章節跟 2026-09-25 的「使用者收藏資料被同步機制清空」那次真實 bug 記錄。

<details>
<summary>原始記錄（點開展開）</summary>
...
沒測到的：真的拿一組 GitHub PAT 連接、在兩台裝置間實際互推/互拉一次——這部分後來也不再相關，整個機制已經被取代。
...
## 這個計畫後來做完了：改成真帳號登入，取代 Gist 同步（規劃於 2026-09-18，完成於 2026-09-20）

✅ **這裡描述的方向已經做完並上線**，見下一節「Supabase 帳號登入上線」的實際結果（跟原計畫有幾處差異，例如只留 Google 登入、拿掉了 email/密碼）。保留這節是因為規劃階段的取捨理由（為什麼選 Supabase）還有參考價值。
...
（以下是規劃當下、2026-09-18 寫的原始狀態，早就過時了——實際完成情形見下一節）
...
- 約 50 位新藝人／品牌補進 `artists.yml`。（當時依國籍把蔡健雅、巫啟賢歸「亞洲其他」，2026-09-24 已依第 12 條新規則改回「本地」。）
...
- **不要在同一台電腦上為了驗證反覆重跑**：今天 Mac 這台跑了 4 輪完整抓取加實驗，幾千次請求很可能就是 403 暴增的原因之一。

**4. ZAZEN to VOOID（9/24 Legacy Taipei）每一輪都消失：兩個 bug 疊在一起**（比對街聲資料時發現）
- Legacy Taipei／Taichung 的場館比對規則要求地址「以台北／台中開頭」，但真實地址常常前面帶郵遞區號（「100臺北市…」），所以被判定成搜尋誤判丟掉。9/21 修城市判斷時已經遇過同一類問題，但這兩條規則沒跟著改。改成 `includes()`，並加上用真實地址寫的測試。
- 站內搜尋策略在比對場地**之前**就把 raw_id 標成「已看過」，所以被丟掉的場次連全站分類掃描都會跳過。改成比對通過才標記；被拒絕的場次先把詳情暫存起來，如果全站分類掃描也掃到它，就直接收錄，不會多發一次請求。
- 這一輪被搜尋策略丟掉的有 23 次，大多是真正的垃圾（算命廣告、杜拜修車），它們不在音樂分類裡，所以修完後還是不會進來。但也混了一些看起來是真實場次的（`eureka2026taipei`、公視 `26boi-*`、`aoi2026tn` 等）。**下一輪跑完之後要確認這些有沒有被撿回來**，進度 log 會出現「recovered via category browse」字樣。
...
2. **算三個比例**（分母都是 `checked`）：
   - 第一次就成功：`ok_first_try / checked`
   - 重試才成功：`ok_on_retry / checked`
   - 兩次都失敗：`failed / checked`
   - 另外參考 `blocked_403`，這是被 Cloudflare 擋的**嘗試次數**，不是場次數，一場最多算 2 次。
3. **依結果決定怎麼改**（改法只動 `scripts/adapters/kktix.mjs` 的 `fetchRegisterStatus()`）：
   - **第一次成功率高（例如 > 70%）**：今天的 403 主要是這台 Mac 反覆重跑造成的，正常排程沒問題。只要看重試有沒有價值就好。
   - **重試才成功接近 0**：重試只是在浪費時間，拿掉重試（`for` 迴圈改成只跑一次），大約再省一半的查詢時間。
...
## 已收錄場次的售票狀態，五個平台都會每天複查（2026-09-24）

**問題**：每個平台的增量抓取都是「收錄過就整筆沿用上次的資料」，狀態也跟著沿用。所以只要是在收錄**之後**才售完、取消或開賣的場次，狀態就永遠不會更新。KKTIX 先補了複查，但 9/24 盤點時，另外四個平台還有 207 場「開賣中」從收錄之後就沒人再看過。

**做法**：每個平台找它最便宜的狀態訊號，已收錄的場次每天查一次。統一換算邏輯放在 `scripts/sale-signal.mjs`（訊號 → 狀態，並略過已售完／還沒到開賣日的場次）。`fetch.mjs` 沿用舊資料時會套上新狀態，進度 log 會印出 `status X -> Y`。

| 平台 | 訊號 | 成本 | 驗證 |
|---|---|---|---|
| KKTIX | 活動頁 JSON-LD `offers[].availability`（`offers` 是空的才退回 register_info） | 普通請求 | 22/22 吻合 |
| Ticket Plus | `config/api/v1/get` 的 session `status`（soldout/over/onsale/pending）。要用的 e/s 編號，是網址上的長串亂碼用 Ticket Plus 自己前端 JS 裡的 AES 金鑰解出來的，跟每個訪客的瀏覽器做的一樣；**Max 9/24 同意這樣做** | 普通請求，約 0.1 秒 | 12/12 吻合 |
| iNDIEVOX | 原本就在用的售票頁 `/activity/game/{id}`（沒有反機器人） | 普通請求 | 抓到 NIEL 已售完 |
| FANSI GO | 票券頁 JSON-LD offers ＋ 每個票種的 `saleEnd`。**注意活動編號不一定等於票券頁編號**（500060 → 500062），要先從活動頁找第一個票券連結 | 普通請求 ×2 | 23 場全查：4 場狀態有變，逐一合理 |
| 拓元 | **沒有便宜訊號**：詳情頁、售票頁用普通請求都回 401。沿用開瀏覽器看售票頁 | 每場約 4 秒 | Stray Kids 正確 |

另外，不管有沒有訊號，只要「尚未開賣」的場次開賣時間已經過了，就會自動轉成開賣中（TAKASE TOYA 那個 bug，對每個平台都適用）。

**順便修掉的舊 bug**：拓元跟 iNDIEVOX 判斷售完的字詞清單沒有「已售完」（Stray Kids 售票頁第一列就是這三個字），新場次也受影響。已改成跟 normalize.mjs 同一套結構化規則（`TICKET_TERMINAL_TEXT_RE`）。

**時間**：四個平台的複查合計約 5 分鐘（拓元佔 3 分多），但各平台是同時跑的，總時間取決於最慢的 KKTIX，所以預估不會讓整體變久。**還沒用完整抓取驗證**，明天 10 點的排程是第一次實測。

## KKTIX 漏抓的完整盤點（2026-09-24）

用瀏覽器把 KKTIX 不篩分類的全部活動清單翻完（65 頁、770 場），逐一檢查「看起來是音樂但 LiveRadar 沒有」的場次，看它們的分類標籤跟場地。

**Max 回報過的漏抓，原因全部查清楚了**：
- 藤井風、高橋洋子、The Notwist、向井太一、XinU：只標「演唱會」或其他分類，當時只掃「音樂」→ 9/23 已改成掃多個分類。
- iri：**完全沒標任何分類**，只有主辦帳號白名單抓得到。
- ZAZEN to VOOID：地址前有郵遞區號被誤判 → 已修。
- **更正**：9/23 以為「羊文学有標音樂分類還是漏掉」，其實羊文学兩場 9/22 就收錄了，從沒漏過。Max 9/23 給的連結 `omiev` 是 **XinU**，當時看錯，還一路把它當成找不到原因的謎。

**這次盤點新發現、原本沒人回報的漏抓**：10 場只標「藝人見面會」(tag 7)（BOY SOMPOB 演唱會、STAYC、韓東 Fan Concert、林鼓子、小宮有紗、ONG SEONGWU、LUDA、OrmFolk、黃致列簽名會、FANKLUB 派對），2 場只標「電音派對」(tag 9)。已把 7、9 加進 `CATEGORY_TAG_IDS`，兩個分類很小（約 6 頁＋1 頁）。

**實際清單大小**（9/24）：音樂 11 頁、演唱會 19 頁、音樂會 6 頁、藝人見面會 6 頁、電音派對 1 頁。清單是照日期排的，過期場次只有第 1 頁幾筆。演唱會 19 頁已經貼著原本 20 頁的上限，上限調到 40（遇到空頁就會停，不增加成本）。

**評估過但沒做**：掃不篩分類的全部清單。770 場裡約 500 場是單口喜劇、課程、展覽、卡牌展，每場第一次都要抓詳情頁，而且沒辨識出藝人的場次照樣會上架，會污染網站。清單卡片上只有很粗的大分類（演出／其他／學習…）。

**還剩的缺口**：像 iri 這種完全沒標分類的場次，卡片大分類會顯示「其他」（152 場，我們只收錄 8 場，大部分不是音樂）。目前只能靠主辦帳號白名單補。如果以後又出現這種漏抓，可以考慮只掃「其他」大分類，再用標題、場地過濾。

另外 Jane Remover（SUB，有標音樂）這次盤點時還沒收錄，用 `fetch-one` 看解析正常，它 9/24 晚上 8 點才開賣，應該是當天中午最後一次抓取之後才上架，下次排程會抓到。

### 9/24 修改的第一次完整實測（2026-09-24 15:08，放假前手動跑）

- **總時間 11 分 6 秒**（優化前約 30 分鐘）。各平台是同時跑的：iNDIEVOX 1 分 19 秒、FANSI GO 1 分 23 秒、Ticket Plus 1 分 45 秒、拓元 6 分 20 秒、KKTIX 11 分 05 秒（最慢的決定總時間）。
- **KKTIX**：262 場靠頁面 JSON-LD 判斷售票狀態；register_info 只剩 30 次（原本約 270 次＋200 次重試），但這 30 次裡還是有 27 次被擋。這 30 場都是 JSON-LD 沒有 offers 的頁面，大多是整場座位制的大型演出，**這幾場的售完偵測仍不可靠**，是目前剩下的缺口。另有 21 場已收錄場次因為狀態改變被重新抓取。
- **其他四個平台的複查**：共查了 224 場，15 場狀態改變。抽查兩場確認正確：May'n 其實 9/27 才開賣，之前被舊規則預設成開賣中；無妄合作社 8/15 已經演完。其他變化包括 NIEL、蔡旻佑、劉若英轉成售完。
- 結果：469 場（新增 31、更新 21），待整理 28 筆。大多是新掃的「藝人見面會」分類帶進來的新藝人。

## 2026-09-24 查不到售票狀態的 15 場：KKTIX「場次總覽頁」

全面抓取時 register_info 失敗的 15 場，用瀏覽器逐一查證，4 場狀態是錯的（囪擊音樂祭、林鼓子見面會、瑋瑋見面會 應為販售中；理想混蛋 10/18 應為售完），已直接更正 `data/events.json`。

**原因**：多數不是一般活動頁，而是 KKTIX 的「場次總覽頁」——本身不賣票，只連到各日期／票種的子活動（`/events/<子活動>/registrations/new`）。總覽頁的 JSON-LD 沒有 offers，register_info 又因為沒有自己的票而回報 SOLD_OUT（林鼓子就是這樣被誤判）。
...
1. ~~**重新查證藝人分類**~~ ✅ 這台 Mac 2026-09-25 完成第一輪。範圍：216 筆完全沒註解＋83 筆只寫「查證為X（WebSearch）」沒依據＝299 筆，其中 232 筆目前真的有場次在用（其餘 64 筆是庫存資料、暫無場次，先擱置）。Max 選的做法：先用 `events.json` 現有的場地／主辦／標題／同場演出者資料交叉比對，篩出真正可疑的再查，不是每筆都重發 WebSearch。
   - 225 筆本地資料就能對上（獨立樂團純台灣小場館巡演、標題本身寫「發片巡迴」「週年」這類本土用語、或大型國際巡演場館/標題明確），補上引用實際資料的依據註解（`2026-09-25 補依據（本地資料交叉比對）`），沒有發任何請求。
   - 7 筆本地資料看不出方向（TOTORRO、D'MASIV、Jony J、Shye、Kiefer、EVILDEAD、MONZEN NO KOZO），另外真的用 WebSearch 查證——**全部 7 筆原本的分類都是對的**，沒有抓到新的誤判，但這次是真的查證，不是本地資料交叉比對。
   - **還沒做的**：64 筆目前沒有場次在用的（純資料庫存貨，不影響畫面，優先度較低）；後續有新場次用到這些藝人時，正常查證流程會自然覆蓋到。另外這次補的「本地資料交叉比對」證據強度不完全一致——有明確訊號的（標題直接寫國籍、只在台灣小場館巡演無海外場次）比較可信；只是「場地+標題」但看不出方向性訊號的（例如已知的國際巡演大牌歌手，只因為場館夠大而沒有進一步查證國籍/成名地）比較弱，之後如果又有人回報這類藝人分類錯，優先重查這批。
2. ~~**KKTIX 沒選分類的活動會漏抓**~~ ✅ 這台 Mac 2026-09-25 完成，新增 Strategy 4（`fetchUnfilteredListingPage`）。
   - **關鍵發現**：9/24 評估過「掃不篩分類的全部清單」並否決，理由是「770場裡約500場是課程/展覽/單口喜劇，每場都要開詳情頁」——這個理由其實不成立。清單頁本身每張卡片就帶著粗分類文字（`<span class="category">`，例如 演出/學習/展覽/同好/其他），**不用開詳情頁就讀得到**。500 場課程/展覽其實各自帶著自己的正確分類（學習/展覽），只有真的完全沒被主辦方分類的才會顯示「其他」——iri 自己的活動頁確認過完全沒有任何分類標籤。所以只要專挑「其他」開詳情頁，不用付 500 場的成本。
   - 實測：清單共 65 頁（跟 9/24 的盤點一致），「其他」比例每頁差很多（33%~78%），無法預測，所以整份 65 頁都要掃，但每頁只讀分類文字不用額外請求，只有真的標「其他」的才開詳情頁（每頁約 4-9 張）。
   - `/events?page=N`（不篩分類）一樣在 Cloudflare 保護下，跟現有的分類掃描用同一套導覽+等待機制，不能用 `fetch()`。
   - **新風險，已加安全網**：抽查「其他」分類發現不少 SEO/牙醫/修車廠的內容行銷垃圾帳號（濫用 KKTIX 建立免費活動頁衝搜尋排名），如果剛好有可解析日期，可能直接上架到正式網站（不像一般噪音只會卡在待整理）。已跟 Max 確認，加了英文商業服務關鍵字到 `NOISE_KEYWORDS`（seo services/dental/numerologist 等），**這份清單不是查完就結束，之後看到新的還要繼續加**，跟其他 NOISE_KEYWORDS 批次同一種維護模式。也抽查到真實但非音樂的內容（基隆國際海洋老鷹嘉年華的親子繪畫工作坊）——這種不會誤判成音樂人，只會安靜卡進待整理清單，不算危險。
   - **還沒用真實完整抓取驗證**：只驗證了 `fetchUnfilteredListingPage` 函式本身能正確運作（真的抓到「其他」卡片、正確過濾垃圾），沒有跑過整套 `npm run fetch` 確認端到端流程（沿用「改完先測試、別一直重跑整套」的教訓）。下次排程跑完後，看 log 裡 `unfiltered listing (其他)` 那行印出的數字，確認有沒有抓到新場次。
3. ~~**全面掃一次 KKTIX 已收錄場次，找出票券表格完全不標狀態的頁面**~~ ✅ 這台 Mac 2026-09-25 完成。寫暫時腳本（跑完即刪，未進 git）對 224 場非已結束的 KKTIX 場次逐一跑 JSON-LD→子活動→票券表格三段判斷（跟 `resolveKnownEvent` 內部一樣的邏輯，只是不落到 register_info），全部用一般 HTTP 請求，沒有動用瀏覽器。
   - **結果：224 場裡只有 4 場（1.8%）真的判斷不出來**，比 9/24 觀察到的「15 場 register_info 失敗」小很多——大部分那 15 場已經被 9/24 加的子活動/票券表格判斷解決了，殘留的只是最頑固的一類。4 場：Fujii Kaze 高雄場、理想混蛋 10/18、YESUNG 10週年高雄場、SUPER JUNIOR-83z 台北場。
   - **查證根因，不是猜的**：直接看 Fujii Kaze 這場的真實頁面——票券表格完全沒有任何「結束販售」字樣，頁面自己寫著「未完成結帳及退回票券預計將於 XX 起準時釋出販售」。這類大型體育場/巨蛋座位制場次的票**會持續回流、重新販售**，KKTIX 本身就沒有幫這種票種設計「永久售完」的二元狀態可以顯示，不是頁面解析漏掉，是這個資訊本來就不存在於一般 HTTP 請求讀得到的地方。
   - **結論：不用再花力氣修**。這 4 場（以及未來同類的大型體育場秀）注定只能靠常被擋的 register_info，接受這個殘留缺口——比起繼續加解析規則，不如把力氣留給 register_info 本身的成功率（見交接任務那節）。
   - 為了做這次量測，把 `kktix.mjs` 內部的 `fetchHtml`／`extractTemplate`／`resolveFromChildEvents`／`extractJsonLdEvent` 改成 export（純粹加可見度，沒有改行為，`npm test` 182 個全過），方便以後同類的唯讀量測腳本直接引用，不用重寫一份。
4. ~~**合併卡片只看第一個來源頁的狀態／Disney 兩場被誤合併**~~ ✅ 這台 Mac 2026-09-25 完成，兩個子問題都是真的 bug，各自修好：

   **Disney 13:00／16:30 誤合併**——查證根因：`dedup.mjs` 原本只有「日間／晚間」兩桶，門檻定在 17:00，13:00 跟 16:30 都 < 17:00，都落進「日間」桶，就被當成同一場合併了。改成真正的時間分群：同一個粗桶內，只要兩個時間相差超過 90 分鐘就再拆開（90 分鐘是為了容忍跨平台對同一場秀寫的時間有落差，例如開放入場時間跟正式開演時間），13:00 跟 16:30 差 210 分鐘會被正確拆開，兩個平台把同一場秀寫成相差 30 分鐘的情況則還是會合併。**特別處理過 id 穩定性**：原本「日間／晚間」兩桶只在真的有兩桶時才會在 id 後面加 `-day`／`-evening` 後綴，這次沒有改變這個規則——只有在同一個桶內部真的抓到需要再拆分的情況，才會加新的後綴；本來就只有一個時間、或本來就正確分成日間/晚間兩桶的場次，id 完全不變。這是刻意的：`favorites`／`excluded` 都是用 `id`當 key（`src/state.js`），隨便換 id 生成規則會讓使用者既有的收藏悄悄消失。

   **合併卡片只看 `sources[0]` 的狀態訊號**——查證根因：`fetch.mjs` 每日複查已知場次時，`reuseSignals.get(event.sources[0].raw_id)` 只查合併卡片的第一個來源，其他來源（VIP／非VIP 分開賣的頁面）當天查到的訊號被完全忽略。改成 `combineSaleSignals()`（新增在 `sale-signal.mjs`）：收集卡片底下**每一個**來源這輪查到的訊號，任何一個還能買（IN_STOCK）就整張卡算能買，都確定賣完才算賣完，跟既有 KKTIX 群組頁（`resolveFromChildEvents`）「任一子活動在賣就算在賣」同一套邏輯。**用現有資料確認過影響範圍不是理論性的**：目前真的有 23 張卡是多來源合併（含 HANDOFF 點名的 TayNewPolcasan VIP/非VIP），這個修正是真的會改變結果，不只是防呆。

   `npm test` 從 182 加到 191（新增 9 個：dedup 的時間分群 3 個 + id 穩定性 2 個、sale-signal 的訊號合併 4 個）。**兩個修正都只用單元測試驗證，沒有跑過整套 fetch 確認端到端**——下次排程跑完，可以看 Disney 是不是真的拆成兩張卡、log 裡有沒有出現合併卡片的狀態變化。

## 🚨 緊急：使用者收藏資料被同步機制清空（2026-09-25，處理第4項途中 Max 回報）

做上面第4項時特別注意 id 穩定性是因為擔心 `favorites`／`excluded` 悄悄失效——結果做到一半 Max 真的回報「我發現我收藏的場次不見了」，一路查證下去發現是完全不同、而且嚴重很多的根因，跟今天在改的 dedup/合併卡片邏輯無關。

**根因**：`defaultPrefs()`（`src/state.js`）對從沒存過 `localStorage` 的裝置，也會把 `updated_at` 標成「現在」。`reconcileSupabaseSync()` 每個頁面載入都會跑一次（`src/app.js` 的 `initTimeline` 等函式），拿這個假的「現在」去跟雲端真正的 `updated_at` 比大小——幾乎一定比較新，於是把這個空白預設值**推上雲端覆蓋掉真實收藏**。**任何登入使用者只要在沒有本機存檔的瀏覽器打開網站（新裝置、清過快取、無痕視窗、或本機資料因任何原因遺失）就會觸發**，不是今天才出現的問題，是同步機制上線（9/20）以來就存在的設計缺陷，只是這次真的被使用者踩到才發現。

**修法**：新增 `hasStoredPrefs()` 判斷這台裝置到底有沒有真的存過資料；從沒存過的裝置一律直接採用雲端資料，完全不進入時間戳記比較、更不可能反過來推送。同步決策邏輯抽成純函式 `decideSyncAction()`，不用 mock `localStorage`／Supabase 就能直接測試（新增 `src/state.test.js`，5 個測試），因為這個專案目前完全沒有測試框架能 mock 瀏覽器 API 或 Supabase client。commit `e932b6a`，**這是今天優先權最高的修正，獨立 commit＋push，沒跟其他工作混在一起**。

**Max 這次的資料救不回來了**——查過他唯一使用的瀏覽器，本機 `localStorage` 也已經是空的（`updated_at: 2026-09-22`），推測是同一個 bug 在某次本機資料遺失（例如清過快取後重新用 Google 登入）時，把空白狀態推上雲端，之後這台裝置本身也再次同步吃到了那份空白資料。查過沒有備份 JSON、Max 也確認沒有其他裝置。Max 的回應是「沒差 不見就算了 但之後不可以不見」——接受這次的損失，但要求徹底不能再發生。

**已知、刻意沒動的殘留風險**：`pushToSupabase()` 本身是無條件 `upsert`，推送前不會先跟雲端核對現況。如果**同時開兩個分頁**，分頁 A 收藏一場、分頁 B（載入時間較早、還不知道 A 的異動）也收藏另一場，B 的推送會直接覆蓋掉 A 的那筆——因為`reconcileSupabaseSync()`只在**頁面載入時**跑一次，不是每次寫入前都重新核對。實務影響很小：每次重新整理／開新分頁都會先同步一次才能操作，只有「兩個分頁同時開著、都沒重新整理」這種窄情況才會踩到。要完全解決需要真正的多裝置合併機制（例如 CRDT），對這個個人專案不成比例，先誠實記錄，不動手改。
