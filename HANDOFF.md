...
- **KKTIX 場地欄位只有地址、沒有場地名稱時，城市判斷會完全失效**（`parseKktixVenue` 把整串地址誤判成場地名稱，`cityFromAddress` 因此收到空字串），2026-09-25 查 city:未知 清單時發現的真實案例（TECHNO BUS，地址明明寫著「台中市」）。還沒修，屬於今天優先修過 1、2 之後排下來的第 3 項。
- **X-Formosa 彩虹音樂節的場地欄位抓到網頁分頁標籤文字**（「活動時程｜表演者｜注意事項」），不是真的場地資訊，需要另外開頁面查證是哪種頁面模板造成的。9/25 同一輪 city:未知 清單查證時發現，還沒修。
- 漁光島（TINY ISLAND 小島沙灘派對）、金滿山露營山莊（奇蹟 MIRACLE FESTIVAL）兩個真實場地沒收錄進 `venues.yml`，單純沒補，不是邏輯錯誤。
