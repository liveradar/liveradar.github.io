...
| FR-33 | 收藏優先於所有排除規則，**永不被過濾掉**（2026-09-21 起：只贏過排除規則，不贏過畫面篩選器如城市/月份/類型/地區/價格——見 §6.3 附註） | P0 |
...
**2026-09-21 修正**：原本的決策樹讓 T2（已收藏）直接跳到 Y1，連 T8（畫面篩選器）都一起跳過——實測發現這樣會造成真實的使用者困惑：Max 把城市篩選器切到「新北」，卻看到一場收藏的台北場次照樣顯示出來。查證 FR-33 的原始文字「收藏優先於**所有排除規則**」，用詞明確是「排除規則」（`prefs.excluded_*`），不是「畫面篩選器」（`viewFilters`，城市/月份/類型/地區/價格，是後來才加的獨立機制）——兩者是不同的東西。修法：收藏依然無條件贏過 T3-T7（排除規則），但現在也要通過 T8（畫面篩選器）才會顯示，不再整批跳過。
...
flowchart TD
    ...
    T1 -->|是| X1[移入「已結束」]
    T1 -->|否| T2{已收藏?}
    T2 -->|是| T8b{通過畫面篩選器?<br/>城市／月份／類型／地區／價格}
    T8b -->|是| Y1[顯示<br/>即使命中排除規則<br/>加註「已收藏，忽略排除」]
    T8b -->|否| X3
