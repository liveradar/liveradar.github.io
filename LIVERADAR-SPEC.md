...
**2026-09-21 修正：文件（此處先前未明說，但抓取邏輯一直沒抓時間）的隱含假設「詳情頁沒有結構化時間」是錯的。** Max 質疑「明明頁面有寫時間」後打開真實節目頁查證，`#intro` 區塊裡其實有 `📅 時間：` 開頭的真實結構化時間（例如「📅 時間：2027/05/01(六) 18:45」），只是 HTML 用 `<span>` 標籤把日期字元拆開，naive 的正則比對不到需要加標籤容忍度。新增 `SHOW_TIME_RE`，跟已經在抓的 `#intro`（票價來源）同一次頁面載入一起抓，`parseTixcraftDate()` 也跟著支援解析附加在 `date_raw` 尾端的 `HH:MM`。注意頁面下方通常還有幾個不相關的「時間：」字樣（預售/開賣時間），`SHOW_TIME_RE` 特意錨定在 📅 emoji 前綴上只抓第一個，避免抓錯。
...
**同一天（2026-09-21）緊接著又修正一個一樣性質的錯誤註解**：上面「日期格式跟拓元完全一樣，沒有時間資訊」也是錯的——實際查證，場館區塊正上方的 sibling 元素就有真實的「YYYY/MM/DD HH:MM」文字，同一次頁面載入抓得到，不需要額外請求。抓取邏輯新增時間擷取，附加到 `date_raw` 尾端，交給拓元共用的 `parseTixcraftDate()` 解析（該函式已經在同一天改成支援解析附加的 `HH:MM`，見 §5.3 的對應修正）。這是本次追查裡第二次證明「adapter 自己的舊註解結論不可信，要打開真實頁面才知道」——第一次是上面這段的場館欄位。
...
**2026-09-21 補上票價格式：`CURRENCY_NUMBER_RE` 沒吃全實際出現的貨幣寫法。** 同一輪查證發現 Ticket Plus 有些活動用「TWD 4,280」「NT4,000」（無 `$`）這種格式，原本只認「NT$」「$」「元」，直接跳過；另有些頁面票價欄位裡完全沒有任何貨幣符號，只有純數字列表（例如「票價｜8,500 / 8,000 / … / 1,500」）。修法：`CURRENCY_NUMBER_RE` 加上 `NT`/`TWD` 前綴；新增 `BARE_NUMBER_LIST_RE` 作為「已確認身處票價標籤內、且找不到任何貨幣標記數字」時的最後手段——只在這個受限情境下才信任裸數字，跟頁面全文關鍵字 fallback 的風險等級不一樣。

### 5.7 `parsePriceFromText` 的全形｜轉半形 bug（2026-09-21，追查時間/票價缺漏時發現）

Max 反覆質疑「還是有很多時間或票價沒有上」，逐一打開真實頁面查證後，在 `normalize.mjs` 找到全 session 最隱蔽的一個 bug：**`PRICE_LABEL_RE`（票價標籤擷取正則）的字元類別只寫了全形「｜」（U+FF5C），但 `normalizeFullwidthAscii()` 會在比對前先把整段全形 ASCII 範圍（`！-～`）轉成半形，「｜」剛好落在這個範圍內，因此在正則比對前就已經變成半形「|」（U+007C）——正則永遠比對不到，靜默退回不精準的關鍵字 fallback（撿到含「預售/現場/單人/雙人」等字樣的頁面）。**

KKTIX／iNDIEVOX／Ticket Plus 三個來源都用「票價｜」這種格式（見 §5.4/5.6 的範例），全部中招。之所以一直沒被抓到：既有測試的票價文字剛好都同時含有會觸發關鍵字 fallback 的字樣，掩蓋了 label 比對路徑本身早就失效的事實——測試綠燈，不代表比對邏輯是靠 label 比對成功的。診斷時因為 ESM `node --input-type=module -e` 環境下 `require` 不可用、import 路徑在專案外又會斷掉，改用一支寫進 scratchpad、直接在專案目錄內執行的除錯腳本才追出這條因果鏈。修法：字元類別加回半形「|」，變成 `[：｜|:]`。

同一輪也把 iNDIEVOX 的 label 比對放寬：「活動地點 ｜」這種標籤跟分隔符號中間多一個空格的寫法，原本要求緊鄰，比對不到；「日期及時間」這種標籤變體也一併納入（跟先前「地點/場地/場館」三選一的修正同一個思路，見上方 §5.4 的既有紀錄）。
...
### 5.11 寬宏／年代端點實測結果（2026-09-26，第 9、10 個來源，同一套售票系統的兩張皮）

**兩個平台是同一套「UTK」售票系統**：寬宏（kham.com.tw）和年代（ticket.com.tw）的網址結構完全相同——`UTK0101_06.aspx?TYPE=1&CATEGORY={code}` 是分類清單頁（頁面上每個節目的 `PRODUCT_ID` 直接寫在 markup 裡），`UTK0201_00.aspx?PRODUCT_ID={id}` 是節目自己的場次表——但兩邊套的 HTML 樣板不一樣，因此寫成 `scripts/adapters/utk.mjs` 一套共用邏輯，`kham.mjs`／`era.mjs` 各自只傳 host 名稱進去建立 adapter。兩個 host 都沒有 robots.txt，plain HTTP 就能抓。

**範圍是 Max 拍板決定的（2026-09-25）**：只收演唱會、音樂劇、戲劇（現代戲劇），不收「音樂」分類——9/25 實測年代「音樂」分類 32 個節目幾乎全是古典/合唱/社區音樂會，不在這個專案的收集範圍內。分類代碼兩邊不一樣：寬宏 `{演唱會:205, 音樂劇:80, 戲劇:116, 親子:129}`，年代 `{演唱會:205, 音樂劇:無此分類, 戲劇:116, 親子:129}`——年代完全沒有獨立的音樂劇分類。分類優先順序：音樂劇 > 戲劇 > 不設 category（跟 OPENTIX 一樣沿用 §3.5 機制），親子分類的節目一律排除（用集合差集排除，不是用標題關鍵字）。

**「立即訂購」按鈕文字永遠不變，唯一的售完訊號是價格數字被劃線**：兩個平台就算所有票都賣完，按鈕照樣顯示「立即訂購」——判斷有沒有賣完只能看價格格內每個數字有沒有被 `<s>`（寬宏，外面通常還包一層 `<font color='lightblue'>`）或 `<del>`（年代，外面包在 `<wbr>...、</wbr>` 裡）包住。寫了一個共用的 `STRUCK_TAG_RE` 正規表達式解析器（不用 DOM walk），先把兩種標籤包住的數字都標記成一個 sentinel 字元再統一去標籤，同一套程式碼處理兩個平台的價格格。

**日期格式兩邊不同**：寬宏是純文字 `"2026/11/21(六)15:00"`（或範圍 `"A(day)HH:MM ~ B(day)HH:MM"`，只取開頭）；年代則是機器可讀的 `<time datetime="2026-11-28"><span class="time">15:00</span></time>` 結構，程式先檢查有沒有這個 `<time>` 元素，有就直接用，沒有才退回文字解析。場館名稱／地址兩邊格式一致（`id` 屬性都是 `...PLACE_NAME`／`...PLACE_ADDRESS` 結尾，年代只是前面多包了一層 ASP.NET 控制路徑），用 cheerio 的 `[id$="PLACE_NAME"]` 屬性尾端比對統一處理。

**一場真實演出常常被拆成好幾個表格列**：不同價位分別是不同的 `PERFORMANCE_ID`（9/26 實測 BIGBANG 高雄場 3 個價位 3 列，全部劃線，正確合併成 1 場 sold_out），或者額外多一列「【輪椅場】」／「【身障輪椅場】」（9/26 實測 XG WORLD TOUR 真實節目：VIP 列＋普通列＋輪椅列，三列同一天同一時間同一場館，正確合併成 1 場，輪椅列的價格排除在合併後的價格聯集之外——除非一場真的只有輪椅列這一種票）。合併規則是「日期＋時間＋場館名稱」相同就是同一場，寫成 `combineRowsIntoSessions()`。

**尚未開賣的節目場次表是空的**：9/26 實測寬宏 `UTK0201_00.aspx?PRODUCT_ID=P1G67G3R`（「三個不老擊敗之人生跑馬燈 LIVE SHOW」）直接被 302 導到另一個完全不同的樣板頁（純文案介紹，沒有任何場次表格），所以這個來源沒有「已公告尚未開賣」這個狀態，只有 on_sale／sold_out——場次表是空的就直接跳過，等它真的上架、下次排程自然會抓到。

**題外話排除一筆年代的「贊助方案」**：年代「戲劇」分類裡有一筆「2026火神祭搖滾區席位 - 星火計畫」，WebSearch 查證是「第二屆國際火舞藝術節《火神祭》」的藝文贊助方案（贊助換座位，不是單場演出票），加進 `NOISE_KEYWORDS`（`"星火計畫"`）排除。另一筆年代疑似案例「英雄傳－武聖關公」查證後是真的交響歌劇舞台演出，維持收錄不動。

**9/26 真實抓取結果**：寬宏 29 個場次、年代 25 個場次，`normalize()` 只排除 3 筆（都是上面那筆星火計畫贊助方案），`groupRuns()` 之後共 43 張卡。逐一對照 5 個計畫裡指名的真實案例：BIGBANG 高雄 sold_out、WILD WILD 三個場館分成台中/高雄/台北三張卡（含 Legacy Taichung 真實地址）、XG WORLD TOUR 輪椅票排除、Jason Mraz 正常收錄、年代「親子演唱會」分類節目確認沒有出現在結果裡——全部正確。

**已知限制，接受但不處理**：寬宏／年代跟其他平台之間沒有跨平台去重——如果同一場演出兩個平台都賣票，`computeId()`（§3.3）靠場館名稱字串比對，「臺北小巨蛋」跟「台北小巨蛋」這種異體字差異會讓兩邊被當成不同場次、各自出一張卡，不會合併成一張。目前沒遇到真實案例，先接受這個限制，之後真的看到重複再處理。
